...
class MediaListService: MediaListServiceProtocol {
    ...
    func setupModelsForMediaList() -> [MediaListModel] {
        return [
            MediaListModel.init(image: .iconDuplicateImages, title: "Duplicate Photos", countingItems: photoLibraryService.getGroupAlbumCount(type: .duplicatePhotos), action: .duplicatePhoto),
            MediaListModel.init(image: .iconSimilarPhotos, title: "Similar Photos", countingItems: photoLibraryService.getGroupAlbumCount(type: .similarPhotos), action: .similarPhotos),
            ...
            MediaListModel.init(image: .iconSimilarVideos, title: "Similar Videos", countingItems: photoLibraryService.getGroupAlbumCount(type: .similarVideos), action: .similarVideos)
        ]
    }
}
